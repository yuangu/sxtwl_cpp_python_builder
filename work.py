...
if os.getenv('BUILDFOR') == "python" :
    if platform.system() == "Linux" and os.getenv("INOS") != 'docker':
        cmd = "docker run -DBUILDFOR=python -DINOS=docker -DTWINE_PASS=$TWINE_PASS -DPUSH_PIP=$PUSH_PIP -v $PWD:/work -w /work -it quay.io/pypa/manylinux_2_24_x86_64 /opt/python/cp36-cp36m/bin/python work.py"
        os.system(cmd)
        exit()
